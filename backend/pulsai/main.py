...
async def lifespan(app: FastAPI):
    ...
    # Start MCP manager
    await mcp_manager_instance.start()
    log.info("MCP system initialized successfully")

    yield

    # Cleanup MCP system
    if mcp_router.mcp_manager:
        log.info("Stopping MCP system...")
        await mcp_router.mcp_manager.stop()
        log.info("MCP system stopped")
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for Pulsai.
